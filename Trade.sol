...
contract Trade {
    ...
    function verifyBuyerSign(address buyer, uint256 tokenId, uint amount, address paymentAssetAddress, address assetAddress, Sign calldata sign) internal pure {
        bytes32 hash = keccak256(abi.encodePacked(assetAddress,tokenId,paymentAssetAddress,amount));
    ...
    function buyAsset(Order calldata order, Sign calldata sign) external returns(bool) {
        ...
        verifyBuyerSign(order.buyer, order.tokenId, order.unitPrice, order.erc20Address, order.nftAddress, sign);
